...
class Dashboard extends CI_Controller {
	...
	public function setting(){
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('username')!="" && $this->session->userdata('password')!="" && $this->session->userdata('jt')=="admin"){
			...
			$crud->set_subject('User');
			$crud->columns('creator','judul','desc','tema','nav');
	...
	public function faskes(){
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('username')!="" && $this->session->userdata('password')!="" && $this->session->userdata('jt')=="admin" || $this->session->userdata('jt')=="operator"){
			$this->session->set_userdata('option','faskes');
			$crud = new grocery_CRUD();
			$crud->set_table('faskes');
			$crud->set_subject('Data Faskes');
			$crud->set_relation('id_category','category','category');
			$crud->display_as('id_category','Jenis Faskes');
			$crud->display_as('kode_faskes','Kode Faskes');
			$crud->required_fields('id_faskes','id_category','lat','lng','faskes','kode_faskes','telp','alamat','keterangan');
			if($this->session->userdata('jt')=="operator"){
				$crud->unset_delete();
			}
			$output = $crud->render();
	...
	public function jenis(){
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('username')!="" && $this->session->userdata('password')!="" && $this->session->userdata('jt')=="admin" || $this->session->userdata('jt')=="operator"){
			...
			$crud->set_subject('Data Jenis Faskes');
			$crud->display_as('category','Jenis Faskes');
			$crud->unset_add()
				 ->unset_edit()
				 ->unset_delete();
			$output = $crud->render();
