      ...
      <aside class="main-sidebar">
        ...
        <section class="sidebar">
          ...
          <ul class="sidebar-menu">
			...
			<?php if($this->session->userdata('jt')=='admin' || $this->session->userdata('jt')=='operator'){ ?>
				<li class="<?php if($this->session->userdata('option')!='dashboard'&&$this->session->userdata('option')!='user'&&$this->session->userdata('option')!='setting'&&$this->session->userdata('option')!='profile') echo 'active';?> treeview">
				  <a href="<?php echo base_url();?>dashboard">
					<i class="fa fa-windows"></i> <span>Data</span>
				  </a>
				  <ul class="treeview-menu">
					<?php if($this->session->userdata('jt')=='admin' || $this->session->userdata('jt')=='operator'){ ?>
					<li <?php if($this->session->userdata('option')=='faskes') echo 'class="active"';?>><a href="<?php echo site_url('dashboard/faskes')?>"><i class="fa fa-map-marker"></i> Faskes</a></li>
					<li <?php if($this->session->userdata('option')=='jenis') echo 'class="active"';?>><a href="<?php echo site_url('dashboard/jenis')?>"><i class="fa fa-tags"></i> Jenis Faskes</a></li>
					<?php } ?>
				  </ul>
				</li>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
			...
			<li><?php 
					if($this->session->userdata('option')=='jenis') echo "Data Jenis Faskes";
					else echo "Data ".ucfirst($this->session->userdata('option'));?></li>
          </ol>
        </section>
        ...
        <section class="content">
          ...
          <div class="row">
            ...
            <section class="col-lg-12 connectedSortable">
			  ...
			  ?>">
                <div class="box-header">
				  ...
				  <?php 
					...
					if($this->session->userdata('option')=='cabang') echo 'fa-building';
					if($this->session->userdata('option')=='jurusan') echo 'fa-plane';
					if($this->session->userdata('option')=='siswa') echo 'fa-book';
					if($this->session->userdata('option')=='profile') echo 'fa-user';
					if($this->session->userdata('option')=='batch') echo 'fa-bookmark';
					if($this->session->userdata('option')=='berkas') echo 'fa-file-text';
					if($this->session->userdata('option')=='on the job') echo 'fa-briefcase';
					if($this->session->userdata('option')=='perusahaan partner') echo 'fa-suitcase';
					if($this->session->userdata('option')=='pembayaran') echo 'fa-money';
					if($this->session->userdata('option')=='sertifikat') echo 'fa-tags';
					if($this->session->userdata('option')=='pelanggar') echo 'fa-book';
					if($this->session->userdata('option')=='p2tl') echo 'fa-exclamation-triangle';
					if($this->session->userdata('option')=='prr') echo 'fa-plug';
					if($this->session->userdata('option')=='jenis') echo 'fa-tags';
					if($this->session->userdata('option')=='faskes') echo 'fa-map-marker';
					if($this->session->userdata('option')=='event') echo 'fa-bullhorn';
				  ?> 
				  <?php
			  ...
			  ?>"></i>
                  <h3 class="box-title">
				  <?php if($this->session->userdata('option')=='prr' OR $this->session->userdata('option')=='p2tl')
					       echo "Data Pelanggar ".strtoupper($this->session->userdata('option'));
					    else
				           echo "Data ".ucwords($this->session->userdata('option'));
					    if($this->session->userdata('option')=='jenis')
				           echo ucwords(' faskes');?></h3>
                  <!-- tools box -->
